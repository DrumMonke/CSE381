...
void renderMt(const Params& prm, int threads, std::vector<uint8_t>& pixels) {
    ...
    for (int t = 0; t < threads; t++) {
        ths.push_back(std::thread(renderCyclicWorker, std::cref(prm),
                        t, threads, std::ref(pixels)));
    }
    for (auto& t : ths) {
        t.join();
    }
...
bool parseSerialArgs(int argc, const std::vector<std::string>& args,
    ...
    // Your code here.
    if (argc != 6) {
        std::cerr << "serial requires WIDTH HEIGHT"
                  << " MAX_ITER OUT.ppm" << std::endl;
        return false;
    }
    out = args[5];
    return true;
...
bool parseMtArgs(int argc, const std::vector<std::string>& args,
    ...
    // Your code here.
    if (argc != 7) {
        std::cerr << "mt requires WIDTH HEIGHT"
                  << " MAX_ITER THREADS OUT.ppm" << std::endl;
        return false;
    } else if (threads < 2) {
...
bool parseArgs(int argc, const std::vector<std::string>& args,
    ...
    if (mode == "serial") {
        ...
        std::cerr << "Mode must be 'serial' or 'mt'." << std::endl;
    }
    // Your code here.
    (void)threads;
    (void)out;
